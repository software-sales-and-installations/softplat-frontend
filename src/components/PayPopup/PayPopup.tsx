...
import {
  CARDNUMBER_VALIDATION_CONFIG,
  VALIDDATE_VALIDATION_CONFIG,
  CVV_VALIDATION_CONFIG,
  CARDNAME_VALIDATION_CONFIG,
...
export const PayPopup: FC = () => {
  const popupState = useSelector(
    (state: RootState) => state.popupOpen.setIsOpened,
  );
  ...
  const onSubmitResData: SubmitHandler<IPayFields> = () => {
    ...
    setTextSubmit(
      'Простите, мы еще не научились это делать. А пока Вы можете перечислить эти деньги в фонд помощи дикой природы',
    );
  };
  useEffect(() => {
    setTextSubmit('');
    ...
    <Popup>
      ...
      <form className={styles.form} onSubmit={handleSubmit(onSubmitResData)}>
        <div className={styles.containerPayPopup}>
          <label
            className={styles.containerPayPopup__label}
            htmlFor="cardNumber"
          >
            Номер карты
          </label>
          ...
          <span className={styles.popup__carderror}>
            {errors?.cardNumber && (
              <p className={styles.popup__errorspan}>
        ...
        <div className={styles.containerForDataCard}>
          <div
            className={classNames(
              styles.containerPayPopup,
              styles.containerPayPopup_type_validData,
            )}
          >
            <label
              className={styles.containerPayPopup__label}
          ...
          >
            <label className={styles.containerPayPopup__label} htmlFor="cvv">
              CVV
            </label>
            <InputMask
              className={styles.input__field}
            ...
            <span className={styles.popup__carderror}>
              {errors?.cvv && (
                <p className={styles.popup__errorspan}>{errors?.cvv.message}</p>
              )}
            </span>
          </div>
        </div>
